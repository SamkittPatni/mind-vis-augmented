...
def get_eval_metric(samples, avg=True, device='cuda'):
    metric_list = ['mse', 'pcc', 'ssim', 'psm']
    res_list = []

    gt_images = [img[0] for img in samples]
    gt_images_np = rearrange(np.stack(gt_images), 'n c h w -> n h w c')
    ...
    for m in metric_list:
        ...
        for s in samples_to_run:
            ...
            pred_images_np = rearrange(np.stack(pred_images), 'n c h w -> n h w c')
            res = get_similarity_metric(pred_images_np, gt_images_np, method='pair-wise', metric_name=m)
            res_part.append(np.mean(res))
        res_list.append(np.mean(res_part))

    # Original top-1-class (mean and max) for 50-way
    res_part = []
    all_trials = []
    for s in samples_to_run:
        pred_images = [img[s] for img in samples]
        pred_images_np = rearrange(np.stack(pred_images), 'n c h w -> n h w c')
        res = get_similarity_metric(pred_images_np, gt_images_np, 'class', None,
                                    n_way=50, num_trials=1000, top_k=1, device=device)
        res_part.append(np.mean(res))
        all_trials.append(res)
    mean_top1 = np.mean(res_part)
    max_top1 = np.max([np.mean(r) for r in all_trials])
    res_list.append(mean_top1)
    res_list.append(max_top1)
    metric_list.append('top-1-class')
    metric_list.append('top-1-class (max)')

    # Additional top-k metrics for 50-way and 100-way
    for n_way in [50, 100]:
        for k in [5] if n_way == 50 else [1, 5]:
            res_part = []
            for s in samples_to_run:
                pred_images = [img[s] for img in samples]
                pred_images_np = rearrange(np.stack(pred_images), 'n c h w -> n h w c')
                res = get_similarity_metric(pred_images_np, gt_images_np, 'class', None,
                                            n_way=n_way, num_trials=1000, top_k=k, device=device)
                res_part.append(np.mean(res))
            res_list.append(np.mean(res_part))
            metric_list.append(f'top-{k}-class-{n_way}way')

    # FID Metric
    pred_images = [img[1] for img in samples]
    pred_images_np = rearrange(np.stack(pred_images), 'n c h w -> n h w c')

    fid_score = get_similarity_metric(pred_images_np, gt_images_np, method='metrics-only', metric_name='fid')
    res_list.append(fid_score)
